...
import { isObject, isString, isUndefined, isNull } from '@react-native-firebase/app/dist/module/common';
import NativeError from '@react-native-firebase/app/dist/module/internal/NativeFirebaseError';
import { parseSetOptions, parseSnapshotArgs, parseUpdateArgs, validateWithConverter, applyFirestoreDataConverter, throwIfLiteSdkSnapshotListenerUnsupported } from "./utils/index.js";
...
export default class DocumentReference {
  ...
  get(options) {
    ...
    return this._firestore.native.documentGet(this.path, options ?? {}).then(data => new FirestoreDocumentSnapshotClass(this._firestore, data, this._converter));
  }
  isEqual(other) {
  ...
  onSnapshot(...args) {
    ...
    const onSnapshotSubscription = this._firestore.emitter.addListener(this._firestore.eventNameForApp(`firestore_document_sync_event:${listenerId}`), event => {
      if (event.body.error) {
        ...
        if (!snapshot) return;
        const documentSnapshot = new FirestoreDocumentSnapshotClass(this._firestore, snapshot, this._converter);
