...
import { revivePipelineValue, configureWebNestedPipelineRevival } from "./pipeline_node_builder.js";
...
function buildSubcollectionSourcePipeline(source) {
  const subcollectionFactory = firebaseFirestorePipelines.subcollection;
...
function applyPipelineStage(current, stage, nestedUnionPipeline) {
  ...
  switch (stage.stage) {
    ...
    case 'search':
      return method.call(current, stageArgs);
    case 'define':
      return Array.isArray(stageArgs.variables) && stageArgs.variables.length > 0 ? method.call(current, ...stageArgs.variables) : method.call(current, stageArgs);
    default:
      return method.call(current, stageArgs);
  }
}
...
export function buildWebSdkPipeline(firestore, request) {
  configureWebNestedPipelineRevival(firestore, (nestedFirestore, nestedPipeline) => buildWebSdkPipeline(nestedFirestore, {
