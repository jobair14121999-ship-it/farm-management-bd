...
function documentSnapshotToObject(snapshot) {
  ...
  if (exists) {
    ...
    out.dataEstimate = objectToWriteable(snapshot.data({
      serverTimestamps: 'estimate'
    }) ?? {});
    out.dataPrevious = objectToWriteable(snapshot.data({
      serverTimestamps: 'previous'
    }) ?? {});
    out.dataNone = objectToWriteable(snapshot.data({
...
export default {
  ...
  transactionBegin(appName, databaseId, transactionId, maxAttempts) {
    return guard(async () => {
      const firestore = getCachedFirestoreInstance(appName, databaseId);
      const transactionOptions = maxAttempts > 0 ? {
        maxAttempts
      } : undefined;
      try {
