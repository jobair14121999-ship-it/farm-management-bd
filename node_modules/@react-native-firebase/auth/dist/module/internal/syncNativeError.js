...
function hasAuthFirebaseErrorCode(error) {
  return isObject(error) && 'code' in error && typeof error.code === 'string' && (error.code.startsWith('auth/') || error.code === INVALID_TOTP_SECRET_CODE);
}
function getNativeErrorUserInfo(error) {
  if (!isObject(error)) {
    return null;
  }
  if ('userInfo' in error && isObject(error.userInfo)) {
 ...
 */
export function rethrowAuthSyncNativeError(error) {
...
//# sourceMappingURL=syncNativeError.js.map