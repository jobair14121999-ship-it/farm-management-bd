...
import type { ActionCodeInfo, ActionCodeSettings, AdditionalUserInfo, Auth, AuthCredential, AuthProvider, ConfirmationResult, IdTokenResult, MultiFactor, MultiFactorAssertion, MultiFactorInfo, MultiFactorResolver, MultiFactorSession, PhoneAuthListener, User, UserCredential } from './auth';
...
export type UserCredentialResultInternal = UserCredential & Partial<Pick<UserCredential, 'operationType' | 'providerId'>>;
...
export interface RNFBAuthModule {
    ...
    verifyPhoneNumberForMultiFactor(phoneNumber: string, session: MultiFactorSession): Promise<string>;
    resolveMultiFactorSignIn(session: MultiFactorSession, verificationId: string, verificationCode: string): Promise<NativeUserCredentialInternal>;
    resolveTotpSignIn(session: MultiFactorSession, uid: string, totpSecret: string): Promise<NativeUserCredentialInternal>;
    ...
    signInWithCredential(providerId: string, token: string, secret?: string | null, fullName?: Record<string, unknown> | null): Promise<NativeUserCredentialInternal>;
    ...
    verifyBeforeUpdateEmail(newEmail: string, actionCodeSettings?: ActionCodeSettings): Promise<NativeUserInternal>;
    ...
    generateTotpSecret(session: MultiFactorSession): Promise<{
        secretKey: string;
    }>;
...
export type AuthInternal = Auth & {
    ...
    getMultiFactorResolver(error: unknown): MultiFactorResolverResultInternal | null;
    isSignInWithEmailLink(emailLink: string): boolean;
    onAuthStateChanged(listenerOrObserver: import('./auth').CallbackOrObserver<AuthListenerCallbackInternal>): () => void;
    onIdTokenChanged(listenerOrObserver: import('./auth').CallbackOrObserver<AuthListenerCallbackInternal>): () => void;
    sendPasswordResetEmail(email: string, actionCodeSettings?: ActionCodeSettings | null): Promise<void>;
    sendSignInLinkToEmail(email: string, actionCodeSettings?: ActionCodeSettings): Promise<void>;
    setLanguageCode(code: string | null): Promise<void>;
    ...
    verifyPhoneNumberWithMultiFactorInfo(multiFactorHint: MultiFactorInfo, session: MultiFactorSession): Promise<string>;
    verifyPhoneNumberForMultiFactor(phoneInfoOptions: import('./auth').PhoneMultiFactorEnrollInfoOptions): Promise<string>;
    verifyPasswordResetCode(code: string): Promise<string>;
    revokeToken(authorizationCode: string): Promise<void>;
    ...
    resolveMultiFactorSignIn(session: MultiFactorSession, verificationId: string, verificationCode: string): Promise<UserCredentialWithAdditionalUserInfoInternal>;
    resolveTotpSignIn(session: MultiFactorSession, uid: string, totpSecret: string): Promise<UserCredentialWithAdditionalUserInfoInternal>;
